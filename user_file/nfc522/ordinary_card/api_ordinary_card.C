#include "api_ordinary_card.H"

#include"..\ordinary_card\api_md5.c"
...
char nfc_work_repeat_init(void)
{
	SpiInit();
	NRSTPD_ON;
	nrf_delay_ms(1);
...
//判断数据是否需要加密
char data_write_to_card(unsigned char addr,unsigned char *pData)
{
	char status = 0 ;
	uint8_t encrpty_data[16];
	uint8_t Write_data[16];
	uint8_t compare_data[16] ={0};
	char error = 0;
	...
	PcdWrite(addr,Write_data);
	PcdRead(addr,compare_data);
	if((memcmp(Write_data,compare_data,16) == 0))
	{
		status = 1;
	}
	else
	{
		for(;error<3;error++)
		{
			PcdWrite(addr,Write_data);
			PcdRead(addr,compare_data);
			if((memcmp(Write_data,compare_data,16) == 0))//&&(status == MI_OK))
			{
				status = 1;
				break;
			}
...
//判断数据是否需要解密
char data_read_from_card(unsigned char addr,unsigned char *pData)
{
	char status = 0 ;
...
//检查卡片是否为复制卡
char check_card_infor(void)
{
	char status = 0 ;
	//对扇区0的数据块0写入数据--验证是否为复制卡
	if(data_write_to_card(PRODUCT_DATA_BLOCK,"Test") == 0)
	{
	#if defined(DEBUG_APP_NFC)
		printf("不是复制卡\r\n");
	#endif								
...
// 写入卡片数据
char write_data_to_card(void)
{
	char status = SET_CARD_ERROR;
#if defined(DEBUG_APP_NFC)
...
// 改变秘钥
char change_scret_to_card(void)
{
	char status = SET_CARD_ERROR;
#if defined(DEBUG_APP_NFC)
	printf("write_data_to_card\r\n");
	...
	if(find_card_mode())
	{
		if(data_write_to_card(WALLET_CTR_ADDR,ScretCode1) == 1)
		{
		#if defined(DEBUG_APP_NFC)
...
char write_money_to_card(unsigned char dd_mode)
{
	char status = SET_CARD_ERROR;
	if(find_card_mode())
	{
...
char set_nfc_wallet_block(void)
{
	char status = 0;
		...
		if(PcdAnticoll(CardSnr) == 0)		// 防冲撞ok
		{
			 if(PcdSelect(CardSnr) == 0)	//选定卡片	
			 {
				...
				if(PcdAuthState(PICC_AUTHENT1A,WALLET_CTR_ADDR,KeyACode1,CardSnr) == 0)
				{
					...
					if(data_write_to_card(WALLET_CTR_ADDR,ctrl_data) == 1)
						status = 1;
				}
