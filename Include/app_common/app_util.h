...
//ins_battery 内部电池获取
static __INLINE uint8_t ins_battery_level_in_percent(const uint16_t mvolts)
{
    uint8_t battery_level;
    ...
    {
        battery_level = 100 - ((4200 - mvolts) * 8) / 100;
    }
    else if (mvolts > 3860)
    {
        battery_level = 87 - ((4060 - mvolts) * 4) / 34;
    }
    else if (mvolts > 3500)
    {
        battery_level = 60 - ((3860 - mvolts) * 2) / 12;
    }
    else
    {
        battery_level = 0;
...
//ext_battery 外部充电宝获取
static __INLINE uint8_t ext_battery_level_in_percent(const uint16_t mvolts)
{
    uint8_t battery_level;
    ...
    else if(mvolts <= 4200)
    {
      ...
      battery_level = (mvolts-4200)*100/(5000-4200);
      if(((mvolts-4200)*100%(5000-4200)) >= ((5000-4200)/2)) battery_level++;		
		}
		
    return battery_level;
}

static __INLINE uint8_t t_battery_level_in_percent(const uint16_t mvolts)
{
    uint8_t battery_level;
    ...
    else if(mvolts <= 3500)
    {
      ...
      battery_level = (mvolts-3500)*100/(4200-3500);
      if(((mvolts-3500)*100%(4200-3500)) >= ((4200-3500)/2)) battery_level++;	
			battery_level += 2;
			if(battery_level >100) return 100;
		}
